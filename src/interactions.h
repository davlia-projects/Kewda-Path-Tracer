...
#if FRESNEL == 1
		float r0 = glm::pow((m.indexOfRefraction - 1.0f) / (m.indexOfRefraction + 1.0f), 2);
		float r = r0 + (1.0f - r0) * glm::pow(1.0f - fabs(glm::dot(normal, pathSegment.ray.direction)), 5.0f);
		if (u01(rng) < r) {
			pathSegment.ray.direction = glm::reflect(pathSegment.ray.direction, normal);
...
#endif
		{
			float refr = pathSegment.insideObject ? m.indexOfRefraction : (1.0f / m.indexOfRefraction);
			pathSegment.ray.direction = glm::refract(pathSegment.ray.direction, normal, refr);
